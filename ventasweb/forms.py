...
class UserRegistrationForm(forms.ModelForm):
    """Formulario simplificado para crear usuarios (clientes y personal) en sistema de ventas"""
    ...
    class Meta:
        model = CustomUser
        fields = [
            "first_name", "last_name", "email", "telefono", "direccion", "cedula",
            "genero", "rol", 
            # Campos específicos para clientes
            "tipo_cliente", "limite_credito", "dias_credito", "descuento_cliente",
            # Campos específicos para vendedores
            "comision_vendedor", "meta_mensual", "zona_venta",
        ]
        ...
        widgets = {
            ...
            "telefono": forms.TextInput(attrs={'class': 'form-control', 'placeholder': '809-555-1234'}),
            "direccion": forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Dirección completa'}),
            "cedula": forms.TextInput(attrs={'class': 'form-control', 'placeholder': '000-0000000-0'}),
            ...
            "descuento_cliente": forms.NumberInput(attrs={
                'class': 'form-control', 
                'placeholder': '0.00',
                'step': '0.01',
                'min': '0',
            ...
            "comision_vendedor": forms.NumberInput(attrs={
                'class': 'form-control', 
                'placeholder': '0.00',
                'step': '0.01',
                'min': '0',
            ...
            "meta_mensual": forms.NumberInput(attrs={
                'class': 'form-control', 
                'placeholder': '0.00',
                'step': '0.01',
                'min': '0'
            }),
            "zona_venta": forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Ej: Zona Norte'}),
        }
        labels = {
            "first_name": "Nombre",
            "last_name": "Apellido",
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['descuento_cliente'].required = False
        self.fields['comision_vendedor'].required = False
        self.fields['meta_mensual'].required = False
        self.fields['zona_venta'].required = False
    ...
    def clean(self):
        ...
        if rol == "Cliente":
            if not cleaned.get("tipo_cliente"):
                cleaned["tipo_cliente"] = "Minorista"  # Valor por defecto
    ...
    def clean_cedula(self):
        ...
        if not cedula:  # Cédula es opcional
            return cedula
            
        user_id = self.instance.pk
        if CustomUser.objects.filter(cedula=cedula).exclude(pk=user_id).exists():
    ...
    def save(self, commit=True):
        ...
        # Si no tiene contraseña, establecer una temporal
        if not user.password:
            user.set_password(f"temp{user.cedula or user.telefono}")
...
# ============================================================================
# CÓDIGO ANTIGUO ELIMINADO - UserRegistrationForm anterior con campos académicos
# Si necesitas referencia, busca en el historial de git
# ============================================================================
...
class UserUpdateForm(forms.ModelForm):
    """Formulario simplificado para actualizar usuarios (clientes y personal) en sistema de ventas"""

    # IMPORTAMOS LOS MISMOS CHOICES DEL FORMULARIO DE CREACIÓN
    GENERO_CHOICES = UserRegistrationForm.GENERO_CHOICES
    ROL_CHOICES = UserRegistrationForm.ROL_CHOICES
    TIPO_CLIENTE_CHOICES = UserRegistrationForm.TIPO_CLIENTE_CHOICES

    genero = forms.ChoiceField(choices=GENERO_CHOICES, required=False)
    rol = forms.ChoiceField(choices=ROL_CHOICES, required=True)
    tipo_cliente = forms.ChoiceField(choices=TIPO_CLIENTE_CHOICES, required=False, label="Tipo de Cliente")
    ...
    class Meta:
        model = CustomUser
        fields = [
            'email', 'first_name', 'last_name', 'genero',
        ...
        widgets = {
            ...
            'descuento_cliente': forms.NumberInput(attrs={'class': 'form-control', 'step': '0.01'}),
            'comision_vendedor': forms.NumberInput(attrs={'class': 'form-control', 'step': '0.01'}),
            'meta_mensual': forms.NumberInput(attrs={'class': 'form-control', 'step': '0.01'}),
            'zona_venta': forms.TextInput(attrs={'class': 'form-control'}),
        }
...
class UserUpdateFormGood(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field in ['tipo_cliente', 'limite_credito', 'dias_credito', 'descuento_cliente',
                     'comision_vendedor', 'meta_mensual', 'zona_venta',
                     'departamento', 'cargo',
                     'contacto_emergencia_nombre', 'contacto_emergencia_telefono',
                     'contacto_emergencia_parentesco']:
    ...
    def clean(self):
        ...
        if rol == 'Cliente':
            if not cleaned_data.get('tipo_cliente'):
                self.add_error('tipo_cliente', 'El tipo de cliente es requerido')
        elif rol == 'Vendedor':
            if not cleaned_data.get('comision_vendedor'):
                self.add_error('comision_vendedor', 'La comisión es requerida para vendedores')
            if not cleaned_data.get('zona_venta'):
                self.add_error('zona_venta', 'La zona de venta es requerida para vendedores')
