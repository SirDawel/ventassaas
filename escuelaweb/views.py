        ...
        if form.is_valid():
            try:
                ...
                if user.rol == 'Estudiante':
                    ...
                    # Filtrar por tipo
                    concepto_mensualidad = conceptos_estandar.filter(tipo='mensualidad').first()
                    concepto_inscripcion = conceptos_estandar.filter(tipo='inscripcion').first()
                    concepto_transporte = conceptos_estandar.filter(tipo='transporte').first()
                    
                    tarifas_creadas = []
                    
                    # Crear tarifa de mensualidad (obligatoria)
                    if concepto_mensualidad:
                        TarifaEstudiante.objects.create(
                            estudiante=user,
                            concepto=concepto_mensualidad,
                            monto=concepto_mensualidad.monto,
                            activo=True
                        )
                    ...
                    if concepto_transporte:
                        TarifaEstudiante.objects.create(
                            estudiante=user,
                            concepto=concepto_transporte,
                            monto=concepto_transporte.monto,
                            activo=True
                        )
                        tarifas_creadas.append('Transporte')
...
def user_update(request, user_id=None):
    ...
    if request.method == "POST":
        ...
        form = (
            UserUpdateForm(request.POST, instance=user)
            if editing
...
def generar_codigos_barras(request):
    ...
    usuarios = usuarios.order_by('rol', 'grado', 'seccion', 'first_name', 'last_name')
    
    # Conteo por rol
    roles_disponibles = ['Profesor', 'Secretaria', 'Administrador', 'Coordinador', 'Bibliotecario', 'Estudiante']
    conteo_roles = {}
...
def historial_asistencia_general(request):
    ...
    estudiantes_total = estudiantes_ids_activos.count()
    estudiantes_masculino = CustomUser.objects.filter(
        id__in=estudiantes_ids_activos,
        genero='M'
    ).count()
    estudiantes_femenino = CustomUser.objects.filter(
        id__in=estudiantes_ids_activos,
        genero='F'
    ).count()
    ...
    for grado in grados_unicos:
        if grado:
            ...
            estudiantes_grado_ids = Matricula.objects.filter(
                ...
                estudiante__rol='Estudiante'
            ).values_list('estudiante_id', flat=True).distinct()
            
            total_grado = estudiantes_grado_ids.count()
            masculino_grado = CustomUser.objects.filter(
                id__in=estudiantes_grado_ids,
                genero='M'
            ).count()
            femenino_grado = CustomUser.objects.filter(
                id__in=estudiantes_grado_ids,
                genero='F'
            ).count()
            
            # Asistencias del mes para este grado (solo estudiantes matriculados)
            asistencias_grado = AsistenciaPersonal.objects.filter(
                fecha__year=anho,
                fecha__month=mes,
                usuario_id__in=estudiantes_grado_ids
            ).count()
            
            presentes_grado = AsistenciaPersonal.objects.filter(
                fecha__year=anho,
                fecha__month=mes,
                usuario_id__in=estudiantes_grado_ids,
                estado='presente'
            ...
            grados_stats.append({
                'grado': grado,
                'total': total_grado,
                'masculino': masculino_grado,
                'femenino': femenino_grado,
                'asistencias': asistencias_grado,
                'presentes': presentes_grado,
    ...
    if fecha_seleccionada:
        ...
        for grado in grados_unicos:
            if grado:
                # Obtener IDs de estudiantes de este grado
                estudiantes_grado_ids_dia = Matricula.objects.filter(
                    materia__curso__anho_escolar=anho_escolar,
                    estudiante__grado=grado,
                    estudiante__rol='Estudiante'
                ).values_list('estudiante_id', flat=True).distinct()
                
                estudiantes_grado_dia = AsistenciaPersonal.objects.filter(
                    fecha=fecha_seleccionada,
                    usuario_id__in=estudiantes_grado_ids_dia
                )
                
                total_dia = estudiantes_grado_dia.count()
                presentes_dia = estudiantes_grado_dia.filter(estado='presente').count()
                masculino_dia = estudiantes_grado_dia.filter(usuario__genero='M').count()
                femenino_dia = estudiantes_grado_dia.filter(usuario__genero='F').count()
    ...
    context = {
        ...
        'asistencias_estudiantes_mes': asistencias_estudiantes_mes,
        'grados_stats': grados_stats,
...
def ver_pagos_estudiante(request, estudiante_id):
    # Esta vista fue reemplazada por facturas_estudiante
    messages.warning(request, 'El sistema de pago simple ha sido reemplazado por el sistema de facturas.')
...
def reportes_ventas(request):
    ...
    from .models import Articulo
    top_articulos = DetalleFactura.objects.filter(
    ...
    todos_articulos = DetalleFactura.objects.filter(
        factura__fecha_emision__gte=fecha_inicio,
        factura__fecha_emision__lte=fecha_fin,
        factura__estado__in=['pagada', 'parcial']
    ).exclude(
    ...
    todos_conceptos = DetalleFactura.objects.filter(
        factura__fecha_emision__gte=fecha_inicio,
        factura__fecha_emision__lte=fecha_fin,
        factura__estado__in=['pagada', 'parcial']
    ).exclude(
    ...
    from .models import ConceptoPago
    top_conceptos = DetalleFactura.objects.filter(
        factura__fecha_emision__gte=fecha_inicio,
        factura__fecha_emision__lte=fecha_fin,
        factura__estado__in=['pagada', 'parcial']
    ).exclude(
    ...
    top_estudiantes = facturas.values(
        'cliente__first_name', 
        'cliente__last_name',
        'cliente__codigo_barras'
    ).annotate(
    ...
    facturas_anterior = Factura.objects.filter(
        fecha_emision__gte=fecha_inicio_anterior,
    ...
    ingresos_productos = DetalleFactura.objects.filter(
        factura__fecha_emision__gte=fecha_inicio,
        factura__fecha_emision__lte=fecha_fin,
    ...
    ingresos_servicios = DetalleFactura.objects.filter(
        factura__fecha_emision__gte=fecha_inicio,
        factura__fecha_emision__lte=fecha_fin,
    ...
    clientes_frecuentes = facturas.values(
        'cliente__first_name', 
        'cliente__last_name',
        'cliente__id'
    ).annotate(
    ...
    mensualidades = DetalleFactura.objects.filter(
        factura__fecha_emision__gte=fecha_inicio,
        factura__fecha_emision__lte=fecha_fin,
        concepto__tipo='mensualidad'
    ).aggregate(
